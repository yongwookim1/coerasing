...
from src.utils import ImageProjModel, IPAdapter, get_attn_processor, IPAttnProcessor, AttnProcessor
import random
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    erased_concepts = [example["erased_concept"] for example in examples]
...
def text_refinement(text_embeddings, image_embeds, image_proj_model, unet):

    key = text_embeddings
    query = image_proj_model(image_embeds)
    value = key

    attention_scores = torch.matmul(query.to(unet.device), key.to(unet.device).transpose(1, 2))
    ...
    attended_image_embedding = torch.matmul(attention_weights, value.to(unet.device))  # Shape: [batch_size, 1, embedding_dim]
...
def main(args):
    ...
    adapter_modules = torch.nn.ModuleList(attn_procs.values())
    ...
    print(f"Successfully loaded weights from checkpoint {ip_adapter_path}")
    # ip_adapter = IPAdapter(unet, image_proj_model, adapter_modules, ip_adapter_path).to(accelerator.device)
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(unet):
                    ...
                    if args.modality == "image":
                        concept_positions = torch.cat([batch["concept_positions"], torch.ones(1, 4, dtype=batch["concept_positions"].dtype, device=batch["concept_positions"].device)], dim=-1)
                        attn_controller.set_concept_positions(concept_positions, GSAM_mask, use_gsam_mask=args.use_gsam_mask)
                        if batch["erased_concepts"][0] in ["naked", "nudity", "erotic", "sexual"]:
                            image_dir = args.image
                            
                        else:
                            image_dir = os.path.join(args.image, batch["erased_concepts"][0])
                        image_list = os.listdir(image_dir)
                        image_path = os.path.join(image_dir, random.sample(image_list, 1)[0])
                        image = Image.open(image_path)
                        image = transform(image.convert("RGB")).to(image_encoder.device)
                        image = image.unsqueeze(0)
                        image_embeds = image_encoder(image).image_embeds
                        # Add the predicted noise to the original latents to get the final image

                        if args.text_guide:
                            
                            text = image_dir.split("/")[-1]
                            text_input_ids = tokenizer(text, return_tensors="pt", padding="max_length", truncation=True).input_ids
                            text_embeddings = text_encoder(text_input_ids.to(unet.device))[0]
                            image_embeds = text_refinement(text_embeddings, image_embeds, image_proj_model, unet)
                        ...
                        # Register hooks **only on unet attention layers**, excluding image_proj_model
                        # def custom_forward_hook(module, input, output):
                        #     attn_controller(output, "image", preserve_prior=args.with_prior_preservation, latent_num=args.latent_num)

                        # for name, module in unet.named_modules():
                        # Only attach hooks to the original UNet attention layers
                            
                        #     if "attn" in name.lower():
                        #         module.register_forward_hook(custom_forward_hook)

                        model_pred = unet(noisy_latents, timesteps, encoder_hidden_states).sample
                        # model_pred = ip_adapter(noisy_latents, timesteps, encoder_hidden_states, image_embeds)
                    else:
