...
def train_image_mode(args):
    ...
    # Define save path
    save_dir = args.save_path or os.path.join("checkpoints", args.modality, args.prompt, args.train_method)
    if args.lora_init_method is not None:
        lora_save_path = os.path.join(save_dir, f"lora_{args.train_method}_im{args.image_number}_ng{args.negative_guidance}_it{args.iterations}")
    ...
    for idx in tqdm(range(args.iterations), desc="[Image Training]"):
        ...
        if (idx + 1) % args.save_iter == 0:
            if args.lora_init_method is not None:
                ...
                print(f"[Checkpoint] Saved LoRA modules at iteration {idx + 1}")
                # Save remained UNet
